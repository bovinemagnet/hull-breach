...
static func cone(width: int = 96, height: int = 112) -> ImageTexture:
	...
	var origin := Vector2(0.0, float(safe_height) * 0.5)
	var beam_length := float(safe_width)
	...
	for y in safe_height:
		for x in safe_width:
			...
			var alpha := angle_fade * distance_fade
			image.set_pixel(x, y, Color(1.0, 1.0, 1.0, alpha))
	return ImageTexture.create_from_image(image)
